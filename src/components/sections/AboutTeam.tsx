import { motion } from "framer-motion"

import { teamMembers } from "@/lib/placeholder-assets"
import { fadeUp, viewportDefault } from "@/lib/motion"

/* ────────────────────────────────────────────────────────────
 * About — Section 5: The Team.
 *
 * Off-white. Asymmetric grid of eight team members. Each card
 * is a portrait + Playfair name + tracked-caps role label.
 * Slight asymmetric column offsets keep it from reading as a
 * uniform 4-up.
 * ──────────────────────────────────────────────────────────── */

export default function AboutTeam() {
  return (
    <section
      aria-label="The Team"
      className="relative bg-paper text-ink"
    >
      <div className="mx-auto max-w-[1600px] px-6 py-24 sm:px-10 sm:py-32 lg:px-14 lg:py-40">
        {/* ── Header ── */}
        <motion.div
          initial="hidden"
          whileInView="visible"
          viewport={viewportDefault}
          variants={{
            hidden: {},
            visible: { transition: { staggerChildren: 0.1 } },
          }}
          className="mb-16 grid grid-cols-1 gap-y-8 lg:mb-24 lg:grid-cols-12"
        >
          <motion.p variants={fadeUp} className="pbm-eyebrow lg:col-span-12">
            04 — The Studio
          </motion.p>

          <motion.h2
            variants={fadeUp}
            className="pbm-display-m lg:col-span-7"
          >
            The Team.
          </motion.h2>

          <motion.p
            variants={fadeUp}
            className="pbm-body max-w-[42ch] lg:col-span-4 lg:col-start-9 lg:pt-6"
          >
            Bookers, producers, scouts and designers — a small studio that
            holds the whole calendar together from a quiet room in Yelahanka.
          </motion.p>
        </motion.div>

        {/* ── Asymmetric grid ── */}
        <motion.div
          initial="hidden"
          whileInView="visible"
          viewport={viewportDefault}
          variants={{
            hidden: {},
            visible: { transition: { staggerChildren: 0.08 } },
          }}
          className="grid grid-cols-2 gap-x-3 gap-y-14 sm:gap-x-4 sm:gap-y-20 lg:grid-cols-12 lg:gap-x-6"
        >
          {teamMembers.map((m, i) => {
            // Asymmetric desktop column placement — 8 members across 12 cols
            // with deliberate offsets for editorial cadence.
            const desktopClass = [
              "lg:col-span-3",
              "lg:col-span-3 lg:pt-16",
              "lg:col-span-3",
              "lg:col-span-3 lg:pt-24",
              "lg:col-span-3 lg:pt-12",
              "lg:col-span-3",
              "lg:col-span-3 lg:pt-20",
              "lg:col-span-3 lg:pt-4",
            ][i]

            return (
              <motion.figure
                key={m.name}
                variants={{
                  hidden: { opacity: 0, y: 32 },
                  visible: {
                    opacity: 1,
                    y: 0,
                    transition: { duration: 1.0, ease: [0.22, 1, 0.36, 1] },
                  },
                }}
                className={desktopClass}
              >
                <div className="relative aspect-[4/5] w-full overflow-hidden bg-ink/5">
                  <img
                    src={m.img}
                    alt={`${m.name} portrait`}
                    loading="lazy"
                    className="absolute inset-0 h-full w-full object-cover grayscale"
                  />
                </div>
                <figcaption className="mt-5">
                  <p className="pbm-display-xs text-ink">{m.name}</p>
                  <p className="pbm-meta-label mt-2">{m.role}</p>
                </figcaption>
              </motion.figure>
            )
          })}
        </motion.div>
      </div>
    </section>
  )
}
